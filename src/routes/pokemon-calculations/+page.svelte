<script>
	...
	const P = new Pokedex();
	P.getPokemonByName(1025, (response, error) => {
		if (!error) {
	...
	$: sTierPokemon = Object.entries(pokemonStats.S_TIER).map(([name]) => ({
		name,
		tier: 'S',
	}));
	$: aTierPokemon = Object.entries(pokemonStats.A_TIER).map(([name]) => ({
		name,
		tier: 'A',
	}));
	$: bTierPokemon = Object.entries(pokemonStats.B_TIER).map(([name]) => ({
		name,
		tier: 'B',
	}));
	$: cTierPokemon = Object.entries(pokemonStats.C_TIER).map(([name]) => ({
		name,
		tier: 'C',
	}));
	$: dTierPokemon = Object.entries(pokemonStats.D_TIER).map(([name]) => ({
		name,
		tier: 'D',
	}));
	$: eTierPokemon = Object.entries(pokemonStats.E_TIER).map(([name]) => ({
		name,
		tier: 'E',
	}));
	$: fTierPokemon = Object.entries(pokemonStats.F_TIER).map(([name]) => ({
		name,
		tier: 'F',
	}));
	...
	function calculateStats(pokemon, level, ivs, evs, nature, stages = {}) {
		...
		const natureMultiplier = stat => {
			if (!nature) return 1;
			return stat === nature.increase
				? 1.1
				: stat === nature.decrease
					? 0.9
					: 1;
		};
		...
		const hp = Math.max(
			Math.floor(
				((2 * pokemon.hp + ivs.hp + Math.floor(evs.hp / 4)) * level) /
					100,
			) +
				level +
				10,
		...
		const calculateStat = (baseStat, statName) => {
			const base =
				Math.floor(
					((2 * baseStat +
						ivs[statName] +
						Math.floor(evs[statName] / 4)) *
						level) /
						100,
				) + 5;
			const natureModified = Math.floor(
				base * natureMultiplier(statName),
			);
			const stageModified = Math.floor(
				natureModified * stageMultiplier(statName),
			);
	...
	$: stats = selectedPokemon
		? calculateStats(
				...
				ivs,
				evs,
				pokemonStats.NATURES[selectedNature],
			)
		: null;

	$: movePower = selectedMove
		? calculateDamage(selectedMove, stats, opposingDefense)
		: 0;
	...
	$: groupedMoves = selectedPokemon
		? getGroupedMoves(selectedPokemon.name)
		: {};
	...
	function getNatureName(increasedStat, decreasedStat) {
		...
		if (natureNames[key]) {
			return increasedStat < decreasedStat
				? natureNames[key].minus
				: natureNames[key].plus;
		}
	...
	$: filteredPokemon = allPokemon.filter(([name, data]) => {
		...
		const matchesTier =
			!Object.values(selectedTiers).some(value => value) ||
			selectedTiers[tier];
	...
	function calculateDamage(move, attackerStats, opposingDefense) {
		...
		const baseDamage =
			Math.floor(
				Math.floor(
					Math.floor(
						((2 * level) / 5 + 2) * move.power * attackStat,
	...
	$: {
		if (selectedPokemon) {
			...
			console.log(
				...
				getMoveDetails(pokemonMoves[selectedPokemon.name]?.[0]),
			);
	...
	// Reactive statement
	$: availableMoves = selectedPokemon
		? getAvailableMoves(selectedPokemon.name)
		: {};
...
>
	...
	<div class="w-full max-w-7xl">
		...
		<div class="mb-4 bg-gray-800 p-4 rounded-lg shadow-md">
			<h2 class="text-lg font-semibold text-yellow-500 mb-4">
				Pokémon Selection
			</h2>
					...
					<div class="flex items-center gap-2">
						...
						<button
							...
							on:click={() => {
								...
								if (singleSelectMode) {
									const selectedTierEntries = Object.entries(
										selectedTiers,
									).filter(([_, isSelected]) => isSelected);
									const firstSelectedTier =
										selectedTierEntries[0]?.[0];
			...
			<div class="space-y-2">
				...
				/>
				<div
					class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-1 max-h-48 overflow-y-auto bg-gray-700 p-2 rounded"
				>
					{#each filteredPokemon as [name, data]}
						...
						>
							<span class="font-medium relative z-10"
								>[{getPokemonTier(name)}] {name}</span
							>
							<div
		...
		<div class="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-4 sm:gap-6">
			...
			<div class="space-y-4 bg-gray-800 p-4 rounded-lg shadow-md">
				...
				<div class="space-y-2">
					<label
						class="block text-sm font-medium text-red-400 text-center"
						>Level: {level}</label
					>
					<input
						type="range"
				...
				<div class="space-y-4">
					<div class="space-y-2">
						<h3
							class="text-sm font-medium text-center text-red-400"
						>
					...
					<div class="space-y-2">
						<h3
							class="text-sm font-medium text-center text-blue-400"
						>
							Effort Values (EVs)
						</h3>
						<div class="grid grid-cols-2 gap-2">
			...
			<div class="space-y-4">
							...
							<table class="w-full text-xs">
								<thead>
									<tr>
										...
										{#each natureGrid.decreased as stat}
											<th
												class="text-center whitespace-nowrap px-1"
											>
												<span
													class="inline-flex items-center justify-center w-full gap-2"
												>
													{#if stat === 'attack'}
														<span
															class="text-red-400"
								...
								<tbody>
									{#each natureGrid.increased as increasedStat, rowIndex}
										<tr>
											<th
												class="text-left whitespace-nowrap px-1"
											>
												<span
													class="inline-flex items-center justify-between w-full gap-2"
												>
											...
											{#each natureGrid.decreased as decreasedStat, colIndex}
												...
												{@const blendedColors = {
													attack_defense:
														'bg-orange-200',
													attack_specialAttack:
														'bg-purple-200',
													attack_specialDefense:
														'bg-rose-200',
													attack_speed: 'bg-red-100',
													defense_specialAttack:
														'bg-emerald-200',
													defense_specialDefense:
														'bg-lime-200',
													defense_speed:
														'bg-amber-200',
													specialAttack_specialDefense:
														'bg-teal-200',
													specialAttack_speed:
														'bg-sky-200',
													specialDefense_speed:
														'bg-cyan-200',
												}}
												{@const colorKey = [
													increasedStat,
													...
													.join('_')}
												<td
													class="p-0.5 text-center border border-gray-700"
												>
													<button
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					...
					<!-- Move Calculator -->
					<div class="space-y-4 bg-gray-800 p-4 rounded-lg shadow-md">
						<h2 class="text-lg font-semibold text-yellow-500 mb-2">
							Move Calculator
						</h2>
						...
						{#if selectedPokemon}
							<div class="space-y-2">
								<label
									class="block text-sm font-medium text-blue-400"
									>Available Moves</label
								>
								<div
									class="max-h-48 overflow-y-auto bg-gray-700 p-2 rounded"
								>
									{#if Object.keys(availableMoves).length > 0}
										{#each Object.entries(availableMoves) as [type, moves]}
											<div class="mb-2">
												<h3
													class="text-sm font-medium mb-1 {getTypeColor(
														type,
													)}"
												>
													{type} Moves
						...
						<div class="grid md:grid-cols-2 gap-4">
							...
							<div class="space-y-2 flex flex-col items-center">
								...
								<div class="relative w-48 h-48">
									{#each typeEffectivenessOptions as option, i}
										{@const angle =
											(i * 360) /
											typeEffectivenessOptions.length}
										{@const radius = 60}
										{@const x =
											Math.cos(
							...
							>
								...
								<div class="w-full">
									<label
										class="text-sm font-medium text-blue-400 mb-2 block text-center"
										>STAB</label
									>
									<button
								...
								<div class="w-full">
									<label
										class="text-sm font-medium text-blue-400 mb-2 block text-center"
									>
										Opposing {selectedMove?.category ===
										'Physical'
											? 'Defense'
											: 'Sp. Defense'}
									</label>
									...
									>
										...
										<input
											...
											on:change={() => {
												...
												opposingDefense =
													opposingDefense; // Trigger reactivity
											}}
										/>
										<button
											class="px-3 py-2 rounded-lg bg-gray-700 hover:bg-gray-600 focus:ring-2 focus:ring-yellow-500"
						...
						{#if selectedMove}
							<div
								class="mt-4 p-4 bg-gray-700 rounded-lg text-center"
							>
